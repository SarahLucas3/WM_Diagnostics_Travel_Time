...
#Travel time data
acute_tt_lsoa<-fread("acute_tt_lsoa.csv") %>% mutate(pt_time=ifelse(pt_time>60, 120, pt_time))%>% mutate(car_time=ifelse(car_time>60, 120, car_time))
cdc_tt_lsoa<-fread("cdc_tt_lsoa.csv")%>% mutate(pt_time=ifelse(pt_time>60, 120, pt_time))%>% mutate(car_time=ifelse(car_time>60, 120, car_time))
...
# Activity total
Activity_leaflet(Activity_by_LSOA, procedure_count, '(procedure)',  "Total imaging procedures")
Activity_leaflet(Activity_by_LSOA,  procedure_count, 'Magnetic' ,  "MRI procedures")
Activity_leaflet(Activity_by_LSOA,  procedure_count, 'Computerized' ,  "CAT procedures")
Activity_leaflet(Activity_by_LSOA,  procedure_count,'radiography' ,  "X-ray procedures")
Activity_leaflet(Activity_by_LSOA,  procedure_count, 'ultrasonography' ,  "Ultrasound procedures")
...
left_join(., West_Mids_Lsoa[,c("geometry","lsoa11")], by=c("lsoa11"))%>%
  st_as_sf()%>%
  filter(!is.na(mri))%>%
  select(-car_distance, -walk_distance, -walk_time)%>%
  gather(., key=mode, value=time, -mri,-cat,-us,-xray, -geometry, -independent, -ic_sitename, -lsoa11)%>%
...
isochrones_CDC <-acute_tt_lsoa %>%
  select(lsoa11, ic_sitename, car_time, pt_time) %>%
  left_join(., activity_per_site, by=c('ic_sitename'))%>%
  filter(!is.na(mri))%>%
  left_join(.,cdc_tt_lsoa [,c("lsoa11","car_time", "pt_time")],by=c("lsoa11") ) %>%
   mutate_all(~replace(., is.na(.), 1)) %>%
  mutate(car_time=pmin(car_time.x, car_time.y), pt_time=pmin(pt_time.x, pt_time.y)) %>%
  select(lsoa11, mri, cat,us, xray, car_time, pt_time)%>%
  gather(., key=mode, value=time, -mri,-cat,-us,-xray, -lsoa11)%>%
  mutate(time=as.numeric(time)) %>%
left_join(., West_Mids_Lsoa[,c("geometry","lsoa11")], by=c("lsoa11"))%>%
  st_as_sf()
...
min_cdc_travel_time<-cdc_tt_lsoa  %>%
  select(-centre, -car_distance, -walk_distance, -walk_time) %>%
  gather("mode", "time",-lsoa11,-name ) %>%
  group_by(lsoa11, mode) %>%
    slice(which.min(time))%>%
  rename(closest_cdc_site=name, closest_cdc_mode=mode, closest_cdc_time=time) 
...
activity_travel_time<-activity %>%
    filter(ic_lsoa %in% West_Mids_Lsoa$lsoa11)   %>%
      filter(ic_sitename %in% activity_per_site$ic_sitename)   %>%
  left_join(.,acute_travel_time,  by=c("ic_lsoa"= "lsoa11", "ic_sitename")) %>%
  left_join(., min_cdc_travel_time, by=c("ic_lsoa"= "lsoa11", "mode"="closest_cdc_mode"))%>%
...
align(., j=1, align = "left")  %>%
  set_header_labels(.,
     ...
     number="Estimated number of procedures where patients don't own a car") %>%
...
GP_decrease_TT<-Travel_time%>%
    filter(did_patsource_code==4) %>%
  filter(cdc_closer==1) %>%
  group_by(ic_modality_desc, mode, pt_accessible )%>%
  summarise(Number=round(sum(procedures_by_transport_mode),0), TT_diff=median(closest_diff, na.rm=TRUE), TT_max=round((min(closest_diff, na.rm=TRUE)),1), TT_min=round((max(closest_diff, na.rm=TRUE)),1), Total=round((sum((closest_diff*procedures_by_transport_mode)/60)),1))%>%
  bind_rows(Travel_time%>%
    filter(did_patsource_code==4) %>%
  filter(cdc_closer==1) %>%
...
GP_decrease_TT2<-Travel_time%>%
    filter(did_patsource_code==4) %>%
  filter(cdc_closer==1) %>%
  group_by(ic_modality_desc, mode, pt_accessible )%>%
  summarise(Number=round(sum(procedures_by_transport_mode),0), TT_diff=median(cdc_diff, na.rm=TRUE), TT_max=round((min(cdc_diff, na.rm=TRUE)),1), TT_min=round((max(cdc_diff, na.rm=TRUE)),1), Total=sum(((cdc_diff)*procedures_by_transport_mode), na.rm=TRUE)) %>%
  bind_rows(Travel_time%>%
    filter(did_patsource_code==4) %>%
    filter(cdc_closer==1) %>%
  group_by(mode, pt_accessible )%>%
  ...
  mutate(TT_range=paste(TT_min, -TT_max, sep=' - '))  %>%
  mutate(ic_modality_desc=ifelse(is.na(ic_modality_desc), "All", ic_modality_desc))%>%
  select(-pt_accessible, -TT_min, -TT_max)%>%
 mutate(mode=ifelse(is.na(TT_diff), "Public transport2", mode)) %>%
...
Unchanged<-Travel_time%>%
  filter(did_patsource_code==4) %>%
filter(closest_diff==0 & ((closest_cdc_site=="Corbett CDC" & ic_sitename=="CORBETT HOSPITAL (RNA04)")|
                      (closest_cdc_site=="Corbett CDC (Guest)" & ic_sitename=="GUEST HOSPITAL (RNA02)")|
                      (closest_cdc_site=="Coventry City Community CDC" & ic_sitename=="COVENTRY CITY CENTRE HEALTH FACILITY (RKB02)")|
                        (closest_cdc_site=="Coventry City Community CDC (Rugby St Cross)" & ic_sitename=="HOSPITAL OF ST CROSS (RKB03)")|
                        ...
                        (closest_cdc_site=="Warwickshire North" & ic_sitename=="GEORGE ELIOT RADIOLOGY (RLT06)")) ) %>%
ungroup()%>%
 # group_by(ic_modality_desc)%>%
  summarise(total=sum(procedures_by_transport_mode))
...
Table3<-flextable(GP_decrease_TT%>% select(ic_modality_desc, Number_Car, TT_diff_Car, TT_range_Car, Total_Car, `Number_Public transport`, `TT_diff_Public transport`, `TT_range_Public transport`,                                                 `Total_Public transport`, `Number_Public transport2`, Total)) %>%
align(.,  align = "center")  %>%
align(., j=1, align = "left")  %>%
  set_header_labels(.,
     ic_modality_desc="Procedure type",
 ...
 add_header_row(values = c("", "By car", "By public transport", ""), colwidths = c(1,4,5,1)) %>% 
  align(., i = 1, part = "header", align = "center")%>% 
...
Table4<-flextable(GP_decrease_TT2%>% select(ic_modality_desc, Number_Car, TT_diff_Car, TT_range_Car, Total_Car, `Number_Public transport`, `TT_diff_Public transport`, `TT_range_Public transport`,`Total_Public transport`, `Number_Public transport2`, Total)) %>%
align(.,  align = "center")  %>%
align(., j=1, align = "left")  %>%
  set_header_labels(.,
     ic_modality_desc="Procedure type",
...
save_as_docx(Table4, path = "Table4.docx")
...
Table_activity_to_cdcs<-Travel_time%>%
  filter(did_patsource_code==4) %>%
    filter((closest_cdc_time<=closest_time) & closest_cdc_time!=120) %>%
  group_by(ic_modality_desc,closest_cdc_site) %>%
  summarise(count=sum(procedures_by_transport_mode)) %>%
...
Procedures_per_site<-Travel_time %>%
  group_by(ic_sitename) %>%
  summarise(Number=sum(procedures_by_transport_mode))


#Table of activity moving from acute sites
Table_activity_from_acute<-Travel_time%>%
  filter(did_patsource_code==4) %>%
    filter((closest_cdc_time<=closest_time) & closest_cdc_time!=120) %>%
group_by(ic_modality_desc,ic_sitename) %>%
  summarise(count=sum(procedures_by_transport_mode))%>%
...
nodes1 <- data.frame(node = c(0:16), 
                    name = c(site),
                    type="acute")
nodes2 <- data.frame(node = c(17:29), 
                    name = c(cdc),
                    type="cdc")
