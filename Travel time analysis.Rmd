...
#Travel time data
acute_tt_lsoa<-fread("acute_tt_lsoa.csv") %>% mutate(pt_time=ifelse(pt_time>60, 120, pt_time))
...
Travel_time<-activity_travel_time %>%
  ...
  mutate(cdc_reduced_TT=ifelse(cdc_diff<0,1,0))%>% # flag LSOA where CDC is closer than the site that was used
    mutate(cdc_diff=ifelse(time==120, NA ,cdc_diff)) %>% #Mark rows with NA where TT is reduced so now under 1 hr
 ...
 mutate(cdc_closer=ifelse(closest_diff<0,1,0))%>% # flag LSOA where CDC is closer than the previous closest site
    mutate(closest_diff=ifelse(closest_time==120, NA ,closest_diff)) %>% #Mark rows with NA where TT is reduced so now under 1 hr
