...
walk_cdc <- fread("tracc/walk_cdc.csv")
car_cdc <- fread("tracc/car_cdc.csv")
PT_cdc <- fread("tracc/PT_cdc.csv")

#Travel time data to Acute Providers
walk_acute<- fread("tracc/walk_acute.csv")
...
#merge
cdc_travel_time<-merge(car_cdc, walk_cdc, by=c('OriginId','DestinationId'), all=T,allow.cartesian=TRUE )


#merge
cdc_travel_time<-merge(cdc_travel_time, PT_cdc, by=c('OriginId','DestinationId'), all=T,allow.cartesian=TRUE )
...
all_cdc_combinations<-left_join(all_cdc_combinations, cdc_tt[,c("pcds","name", "car_time", "car_distance", "pt_time", "walk_time", "walk_distance")], by=c("pcds", "name"))
